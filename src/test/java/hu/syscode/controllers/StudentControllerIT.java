...
public class StudentControllerIT {
	...
	public void testCreateStudent() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/students").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(testStudent))).andExpect(status().isOk())
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/students/" + studentId).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.id").value(studentId.toString())).andExpect(jsonPath("$.name").value(name))
